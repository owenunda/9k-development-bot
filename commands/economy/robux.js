...
function RobuxTradeRate(msg, Bot) {
    ...
    Embed.Description = `Current conversion rate. ${Bot.Shop.Bank.RobuxTradeRate} (1000 Bot Point's = ${(1000 * Bot.Shop.Bank.RobuxTradeRate)})
...
The bank has ${Bot.Shop.Bank.Robux} Robux to trade (you can trade ${(Bot.Shop.Bank.Robux / Bot.Shop.Bank.RobuxTradeRate)} bot point's and must trade at least ${(10 / Bot.Shop.Bank.RobuxTradeRate)})
if you dont want to trade right now just enter 0 to cancel.`;
    msg.channel.send({ embeds: [CreateEmbed(Embed)] }).then(Sent => {
        const msg_filter = response => { return response.author.id === author.id };
        Sent.channel.awaitMessages({ filter: msg_filter, max: 1 }).then((collected) => {
            const Trade = Math.floor(collected.first().content);
            if (user.cash >= Trade && (Trade * Bot.Shop.Bank.RobuxTradeRate) <= Bot.Shop.Bank.Robux && Trade >= (10 / Bot.Shop.Bank.RobuxTradeRate)) {
                const Embed = structuredClone(Bot.Embed);
...
export default {
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('trade')
                .setDescription('Trade bot cash for Robux')),
    aliases: ['!9k Robux Rate', '!9k Robux Trade Rate', '!9k Conversion', '!9k Trade', '!9k Trade Robux', '!9k Robux Trade', '!9k Buy Robux'],
    execute(msg, User, Bot) {
        ...
        if (SearchString(msg.content, ['!9k Robux Rate', '!9k Robux Trade Rate', '!9k Conversion', '!9k Trade'])) {
            RobuxTradeRate(msg, Bot);
        } else if (SearchString(msg.content, ['!9k Trade Robux', '!9k Robux Trade', '!9k Buy Robux'])) {
            RobuxTrade(msg, User, Bot);
        }
    }
}
